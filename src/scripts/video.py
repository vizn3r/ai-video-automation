...
import scripts.subtitles as subtitles
from scripts.apps import Reddit
from scripts.utils import Info, Error, Except, END, CheckMain
from scripts.meta import VideoMeta
from scripts.ai import RedditVideo
...
VIDEO_INPUT_DIR = os.environ["VIDEO_INPUT_DIR"]
...
def main():
    ...
    text_segmets = subtitles.generate(audio_path)["segments"]
    ...
    vid_subtitles = mp.CompositeVideoClip(clips)

    Info("Putting it all together")
    out = mp.CompositeVideoClip([short, vid_subtitles.set_position(("center", "center"))]).set_duration(short.duration)
